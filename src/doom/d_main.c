...
// int             cn_secretmessage = 0; // [crispy] -> [cndoom]
...
void D_Display (void)
{
	...
	if (automapactive)
	    AM_Drawer ();
	if (wipe || (scaledviewheight != (200 << hires) && fullscreen) ) // [crispy] -> [cndoom]
	    redrawsbar = true;
	if (inhelpscreensstate && !inhelpscreens)
	    redrawsbar = true;              // just put away the help screen
	ST_Drawer (scaledviewheight == (200 << hires), redrawsbar ); // [crispy] -> [cndoom]
    ...
    if (gamestate == GS_LEVEL && !automapactive && scaledviewwidth != (320 << hires)) // [crispy] -> [cndoom]
    {
	...
	if (automapactive)
	    ...
	    y = (viewwindowy >> hires)+4; // [crispy] -> [cndoom]
	V_DrawPatchDirect((viewwindowx >> hires) + ((scaledviewwidth >> hires) - 68) / 2, y,
                          W_CacheLumpName (DEH_String("M_PAUSE"), PU_CACHE)); // [crispy] -> [cndoom]
    }
...
//    M_BindVariable("cn_secretmessage",       &cn_secretmessage); // [crispy] -> [cndoom]
