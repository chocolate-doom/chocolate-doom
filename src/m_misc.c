...
wchar_t* M_ConvertToUtf8(const char *str)
{
    ...
    if (!wstr)
    {
        I_Error("M_ConvertToUtf8: Failed to allocate new string");
        return NULL;
    }
...
FILE* M_fopen(const char *filename, const char *mode)
{
    ...
    wname = M_ConvertToUtf8(filename);
