...
visplane_t *R_CheckPlane(visplane_t * pl, int start, int stop)
{
    ...
    {
        if (pl->top[x] != 0xffff) // [cndoom] hires
        {
            break;
        }
    }
...
void R_DrawPlanes(void)
{
    ...
    int skyTexture2;
    int scrollOffset;
    int frac; // [cndoom] hires
    int fracstep = FRACUNIT >> hires; // [cndoom] hires
    ...
    for (pl = visplanes; pl < lastvisplane; pl++)
    {
                ...
                for (x = pl->minx; x <= pl->maxx; x++)
                {
                    ...
                    if (dc_yl <= dc_yh)
                    {
                        ...
                        angle = (viewangle + xtoviewangle[x])
                            >> ANGLETOSKYSHIFT;
                        source = R_GetColumn(skyTexture, angle + offset); // [cndoom] hires
                        source2 = R_GetColumn(skyTexture2, angle + offset2); // [cndoom] hires
                        dest = ylookup[dc_yl] + columnofs[x];
                        frac = SKYTEXTUREMIDSHIFTED * FRACUNIT + (dc_yl - centery) * fracstep; // [cndoom] hires
                        do
                        {
                            if (source[frac >> FRACBITS]) // [cndoom] hires
                            {
                                *dest = source[frac >> FRACBITS]; // [cndoom] hires
                                frac += fracstep; // [cndoom] hires
                            }
                            else
                            {
                                *dest = source2[frac >> FRACBITS]; // [cndoom] hires
                                frac += fracstep; // [cndoom] hires
                            }
                            dest += SCREENWIDTH;
                        }
        ...
        pl->top[pl->maxx + 1] = 0xffff; // [cndoom] hires
        pl->top[pl->minx - 1] = 0xffff; // [cndoom] hires
