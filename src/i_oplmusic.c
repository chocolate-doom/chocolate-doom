...
static void I_OPL_PlaySong(void *handle, boolean looping)
{
    ...
    tracks = X_AllocArray(opl_track_data_t, MIDI_NumTracks(file));
...
void I_OPL_DevMessages(char *result, size_t result_len)
{
    ...
    if (num_tracks == 0)
    {
        X_snprintf(result, result_len, "No OPL track!");
        return;
    }

    X_snprintf(result, result_len, "Tracks:\n");
    lines = 1;
