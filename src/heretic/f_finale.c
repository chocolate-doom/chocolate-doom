...
void F_DemonScroll(void)
{
    ...
    if (finalecount < 70)
    {
        V_CopyScaledBuffer(I_VideoBuffer, p1, ORIGHEIGHT * ORIGWIDTH); // [cndoom] hires
        nextscroll = finalecount;
        return;
    }
    if (yval < 64000)
    {
        V_CopyScaledBuffer(I_VideoBuffer, p2 + ORIGHEIGHT * ORIGWIDTH - yval, yval); // [cndoom] hires
        V_CopyScaledBuffer(I_VideoBuffer + (yval << (2 * hires)), p1, ORIGHEIGHT * ORIGWIDTH - yval); // [cndoom] hires
        yval += ORIGWIDTH; // [cndoom] hires
        nextscroll = finalecount + 3;
    }
    else
    {                           //else, we'll just sit here and wait, for now
        V_CopyScaledBuffer(I_VideoBuffer, p2, ORIGWIDTH * ORIGHEIGHT); // [cndoom] hires
    }
}
